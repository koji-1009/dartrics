...
* **Unused detector follows the operator element on compound assignments.** A user-defined binary operator whose only call site is the compound form (`acc += v` dispatching `operator +`) was flagged as unused — and, by cascade, so were fields read only inside the operator's body. The `AssignmentExpression` hook now records the operator edge, matching the `BinaryExpression` / `PrefixExpression` / `PostfixExpression` handling from 0.7.3. **Stale-dismissal note:** `// dartrics:dismiss unused` comments suppressing the prior false positive may now be reported as stale — remove them.
* **`dartrics analyze` runs the unused pass over generated files, matching `dartrics unused`.** The two commands disagreed: `analyze` dropped `.g.dart` (and friends) from the analysis graph, so a declaration whose only caller lives in a generated file was flagged unused by `analyze` but kept by `unused`. Metric records, `signals:`, snapshot hashes, and the analyzed-file count still exclude generated files — only the reachability graph gains the edges.
* **Under `excludeExported`, members inherited by an exported class from project-internal supertypes are reachability roots.** External consumers can call a superclass or mixin member through the exported subtype, but only the subtype's *declared* members were rooted — an implementation base under `lib/src/` had its members flagged unused. Public instance methods, fields, getters, and setters along the exported type's supertype chain now survive; static and private supertype members still need a caller of their own.
* **Cyclomatic complexity counts switch-expression arms.** `switch` expression arms (`SwitchExpressionCase`) were invisible to CC — a non-exhaustive `switch (x) { 0 => …, 1 => …, _ => … }` scored 1. Arms now count like switch-statement cases; the bare `_ =>` wildcard arm (untyped, unguarded) is the `default:` equivalent and is not counted.
...
* Dependency floors raised: Dart SDK `^3.11.0`, `analyzer ^14.0.0`, `analysis_server_plugin ^0.3.19`, `analyzer_testing ^0.3.3` (dev). The declared SDK floor was `^3.10.0` while analyzer's own floor had already moved to `^3.11.0`, so 3.10 never resolved. `analysis_server_plugin` and `analyzer_testing` pin analyzer to an exact version, so their floors are the releases that pin 14.0.0 — the whole analyzer 14.x line stays admissible. The deprecated `Folder.getChildAssumingFile` API is replaced with its successor `getFile`; behaviour is unchanged.
