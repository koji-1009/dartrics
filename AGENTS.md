...
- `lib/src/cli/` — `CommandRunner` plus the `analyze` / `unused` / `report` / `rules` / `regression` / `manual` subcommands and the shared option set. `snapshot.dart` and `git_diff.dart` are the storage / VCS adapters. `manual_text.dart` is a const-string mirror of `doc/manual.md`; the parity test in `test/cli/manual_command_test.dart` enforces byte equality so they cannot drift.
