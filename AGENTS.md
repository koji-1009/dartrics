...
- `lib/src/cli/` — `CommandRunner` plus the `analyze` / `unused` / `report` / `rules` / `regression` / `manual` / `doctor` subcommands and the shared option set. `snapshot.dart` and `git_diff.dart` are the storage / VCS adapters. `manual_text.dart` is a const-string mirror of `doc/manual.md`; the parity test in `test/cli/manual_command_test.dart` enforces byte equality so they cannot drift. `doctor_command.dart` exports a pure `diagnose(Config)` so embedders can reuse the same validation rules without spawning the CLI.
...
- `lib/src/models/` — `AnalysisReport`, `MetricRecord`, `MetricViolation`, `ScopeRef`, `UnusedDeclaration`, `SourceLocation`, `RegressionReport` family, `AnalyzedFile`, `ExplainEntry`. `ExplainEntry` is the auto-explain payload (rationale + refactorHints + references) that the AI / md / SARIF reporters embed per fired metric. Stable JSON schema lives here.
