...
- `lib/src/cli/` — `CommandRunner` plus the `analyze` / `unused` / `report` / `rules` / `regression` subcommands and the shared option set. `snapshot.dart` and `git_diff.dart` are the storage / VCS adapters.
