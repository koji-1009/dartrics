...
///
/// Off by default: by definition method length = SLOC + blank lines
/// + comment-only lines, so SLOC dominates the score in any
/// reasonably-formatted code; emitting both lenses on the same scope
/// produces redundant violations. Opt in when you specifically want
/// the "how big is this thing on screen" reading (which counts blanks
/// + comment lines) on top of SLOC's "actual code volume" reading.
class MethodLength extends FunctionMetric {
  ...
  String get rationale =>
      ...
      '50–80 lines. Off by default because by definition method '
      'length = SLOC + blank lines + comment-only lines, so SLOC '
      'dominates the score in any reasonably-formatted code; emitting '
